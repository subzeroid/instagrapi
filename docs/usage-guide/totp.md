...
The separate account-recovery UI used by some accounts is not automated. Native CAA exceptions propagate from `login()`. If CAA returns neither a usable session, a supported verification context, nor an explicit fallback instruction, `login()` raises `ClientError` with the CAA failure reason. The explicit `login_legacy()` entry point can fall back to CAA. Errors from that CAA attempt propagate unless the endpoint is unavailable; see [login migration](login-migration.md#keep-the-previous-login-flow).
